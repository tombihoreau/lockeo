...
export class MakeUserLoginNullable20260123120000 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query('ALTER TABLE `users` MODIFY `login` varchar(100) NULL');
    ...
    await queryRunner.query('ALTER TABLE `users` ADD UNIQUE INDEX `UQ_users_login` (`login`)');
  }

  public async down(queryRunner: QueryRunner): Promise<void> {
    // Revenir à NOT NULL (⚠️ peut échouer si des lignes ont login=NULL)
    await queryRunner.query('ALTER TABLE `users` MODIFY `login` varchar(100) NOT NULL');
